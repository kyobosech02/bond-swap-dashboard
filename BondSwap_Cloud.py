...
if check_password():
    ...
    bond_rename_dict = {m: f'{m}_Bond' for m in t1_maturities_list}
    df_bond.rename(columns=bond_rename_dict, inplace=True)

    irs_dfs = []
    for m in t1_maturities_list:
    ...
    b1_rename = {m: f'{m}_B1' for m in t2_maturities_list}
    b2_rename = {m: f'{m}_B2' for m in t2_maturities_list}
    ...
    with tab1:
        ...
        if final_df.empty:
            ...
            multi_columns = pd.MultiIndex.from_product([t1_maturities_list, ['채권(%)', 'IRS(%)', 'Spread(bp)']], names=['만기', '구분'])
            ...
            with chart_col1:
                ...
                fig1.add_trace(go.Scatter(x=chart_df_t1['일자'], y=chart_df_t1[f'{selected_mat}_Bond'], name=f"채권금리 ({selected_bond})", line=dict(color='#2E86C1', width=2), hovertemplate="%{y:.3f}%<extra></extra>"), secondary_y=False)
                ...
                with exp_col2:
                    with st.expander("📊 기초통계값"):
                        ...
                        stat_df = chart_df_t1[['일자', target_col]].dropna().copy()
            ...
            with chart_col2:
                ...
                for m in t1_maturities_list:
                    ...
                    curve_data['Bond'].append(latest_row[f'{m}_Bond'])
                ...
                fig2.add_trace(go.Scatter(x=t1_x_numeric, y=curve_data['Latest'], mode='lines+markers', name=f"최근({latest_date.strftime('%m/%d')})", line=dict(color='red', width=2.5)), secondary_y=True)
                fig2.add_trace(go.Scatter(x=t1_x_numeric, y=curve_data['Avg'], mode='lines+markers', name="평균(Avg)", line=dict(color='green', dash='dash')), secondary_y=True)
                fig2.add_trace(go.Scatter(x=t1_x_numeric, y=curve_data['Max'], mode='lines+markers', name="최대(Max)", line=dict(color='blue', dash='dot')), secondary_y=True)
                fig2.add_trace(go.Scatter(x=t1_x_numeric, y=curve_data['Min'], mode='lines+markers', name="최소(Min)", line=dict(color='purple', dash='dot')), secondary_y=True)
                ...
                fig2.update_xaxes(tickvals=t1_x_numeric, ticktext=t1_maturities_list, showgrid=True, gridwidth=1, gridcolor='LightGray')
                ...
                with st.expander("📊 만기별 Spread 상세 데이터 및 기록 날짜"):
                    display_curve_df = pd.DataFrame(index=t1_maturities_list)
    ...
    with tab2:
        ...
        if t2_final_df.empty:
            ...
            df_t2_table = pd.DataFrame(index=table_rows_t2, columns=t2_maturities_list)
            ...
            with t2_chart_col2:
                ...
                fig_t2_rt.add_trace(go.Scatter(x=t2_x_numeric, y=t2_curve['Spread_Latest'], mode='lines+markers', name="최근 Spread(bp)", line=dict(color='red', width=2.5), hovertemplate="%{y:.1f} bp<extra></extra>"), secondary_y=True)
                fig_t2_rt.add_trace(go.Scatter(x=t2_x_numeric, y=t2_curve['Avg'], mode='lines+markers', name="평균(Avg)", line=dict(color='green', dash='dash'), hovertemplate="%{y:.1f} bp<extra></extra>"), secondary_y=True)
                fig_t2_rt.add_trace(go.Scatter(x=t2_x_numeric, y=t2_curve['Max'], mode='lines+markers', name="최대(Max)", line=dict(color='blue', dash='dot'), hovertemplate="%{y:.1f} bp<extra></extra>"), secondary_y=True)
                fig_t2_rt.add_trace(go.Scatter(x=t2_x_numeric, y=t2_curve['Min'], mode='lines+markers', name="최소(Min)", line=dict(color='purple', dash='dot'), hovertemplate="%{y:.1f} bp<extra></extra>"), secondary_y=True)
                ...
                fig_t2_rt.update_xaxes(tickvals=t2_x_numeric, ticktext=t2_maturities_list, showgrid=True, gridwidth=1, gridcolor='LightGray')
                ...
                with st.expander("📊 만기별 Spread 상세 데이터 및 기록 날짜"):
                    t2_rt_df = pd.DataFrame(index=t2_maturities_list)
